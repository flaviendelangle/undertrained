...
/**
 * The width (in seconds per kilometre) of each bar in the Pace → Distribution
 * histogram, persisted to localStorage so a runner's preferred granularity
 * sticks across activities and sessions. Backed by an external store read
 * through `useSyncExternalStore` (SSR-safe, and kept in sync across every
 * instance) — the same pattern as `usePowerSliceWidth`.
 */
const STORAGE_KEY = "undertrained:pace-slice-width";

export const DEFAULT_SLICE_WIDTH = 15;
export const MIN_SLICE_WIDTH = 5;
export const MAX_SLICE_WIDTH = 60;
 ...
 * of use (see PaceCard), never on the stored value.
 */
...
export function usePaceSliceWidth(): [number, (next: number) => void] {
