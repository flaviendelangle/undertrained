...
- Speak English and French, following the website's en-GB and fr-FR wording. The language comes from an explicit choice made in the app, then the account, then the operating system, then English; the EN/FR switch in the header works before and after sign-in and applies at once. Built-in workout names come from the server in the chosen language; personal workout and device names are never translated.
