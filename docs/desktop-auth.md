...
### Exact workout playback

The workout endpoint also returns an optional `execution` object for each personal or built-in workout. It contains `referenceFtp`, `ftpTest` and a flat `segments` array. Each segment carries `durationSeconds`, `startWatts`, `endWatts`, `cadence`, `note` and `intensity`. Null watt endpoints mean a free step. Repeats are expanded with the website's `flattenWorkout`; ramp endpoints remain exact rather than being averaged like preview bars. Relative targets use the rider's current FTP timeline, and absolute watt targets remain unchanged. Invalid stored structures return `execution: null` and remain preview-only.

This is an additive response change. Older desktop clients ignore it, and newer clients can still display libraries from older servers without pretending that preview bars are executable steps. The desktop currently plays FTP-test steps but does not calculate a result or detect ramp-test exhaustion.
